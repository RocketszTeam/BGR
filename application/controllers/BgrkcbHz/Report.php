...
class Report extends Core_controller
{
    ...
    */
    ...
    public function cm($root = 0)
    {
        ...
        $game_name = 'cm';
        $report_table = 'cm_report';    //報表資料表名稱

        $bet = 'bet_amount';
        $winOrLoss = 'win_amount';
        $valid = 'set_amount';
        $buildDate = 'set_date';
        ...
        if (tb_sql('u_power', 'admin', $root) < $this->web_root_u_power && $this->web_root_u_power != 7 && $this->web_root_u_power != 8) {
            $root = 0;
            ...
            if (($root > 0 && (tb_sql("u_power", "admin", $root) == 6)) || @$_REQUEST["find9"] != '') {
                ...
                if ($rowAll != NULL) {
                    foreach ($rowAll as $row) {
                        ...
                        $data["betAmount"] = ($row["TotalbetAmount"] != "" ? $row["TotalbetAmount"] : 0);
                        $data["winOrLoss"] = ($row["TotalwinOrLoss"] != "" ? $row["TotalwinOrLoss"] : 0);
                        $data["validAmount"] = ($row["validAmount"] != "" ? $row["validAmount"] : 0);
                        $data["totals"] = ($row["totals"] != "" ? $row["totals"] : 0);
                        array_push($dataList, $data);
                    }
                }
    ...
    //新濠報表明細(game_maker = 62)
    public function cm_details($admin_num)
    {
        $game_name = 'cm';
        $report_table = 'cm_report';    //報表資料表名稱
        $bet = 'bet_amount';
