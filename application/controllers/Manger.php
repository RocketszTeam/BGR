...
class Manger extends Core_controller{
	...
	public function __construct(){
		parent::__construct();
		$this->load->library('memberclass');	//載入會員函式庫1
	...
	public function update_sms_token() {
		$token = md5(uniqid(rand(), true));	//手機簡訊token
		$this->session->set_userdata('sms_token', $token);
	...
	public function transfer(){
		...
		if($logMsg!=NULL){
			...
			/*
			//遊戲廠商下拉
			$sqlStr="select * from `game_makers` where num in(3,8,9,12,13,21,22,26,27,28,30,35,36,38,51) and `online`=1 order by `range`";	//排除黃金俱樂部 and 捕魚機
			$this->data["makers_data"]=$this->webdb->sqlRowList($sqlStr);
			$this -> load -> view("www/transfer.php", $this -> data);
			*/
			//轉入
			$sqlStr="select * from `game_makers` where `online`=1 and `deposit` = 'Y' order by `range`";
			$this->data["makers_data"]=$this->webdb->sqlRowList($sqlStr);
			//轉出
			$sqlStr="select * from `game_makers` where `online`=1 and `withdraw` = 'Y' order by `range`";
			$this->data["makers_data2"]=$this->webdb->sqlRowList($sqlStr);
			$this -> load -> view("www/transfer.php", $this -> data);
		}
	}
		...
		if($logMsg!=NULL){
			...
			if(!$this->agent->is_referral()){
				if($this->input->post('amount') && $this->input->post('payment')){
					...
					@$bank=$this->input->post('bankcode',true);
					if(preg_match('/^[0-9]*[1-9][0-9]*$/',$amount)){
						...
						if((int)$amount > 100000 && $payment=='ATM'){
							$this -> session -> set_flashdata("alertMsg",'ATM繳費上限金額不得超過10萬');
							scriptMsg('',"Manger/deposit");
							exit;
						}
						...
						if($order_no!=NULL){
                            ...
                            $sqlStr="select * from `payment_config` where `m_group`=? AND `enable`=? AND `paymentType`=?";
                            $row = $this->webdb->sqlRowCount($sqlStr,array(
                                ':m_group'=>$this->data['user_info']['m_group'],
                                ':enable' => 1,
                            ...
                            if(!isset($row) || empty($row)){
                                if($payment == 'ATM'){
                                    $paymentFunc = 'atm';
                                    $paymentType = 'atm';
                                } else if($payment == "IBON" || $payment == "FAMI" || $payment == "CVS"){
                                    $paymentFunc = 'cvs';
                                    $paymentType = 'cvs';
                                }
                            }

                            $row=$this->webdb->sqlRow($sqlStr,array(
                                ':m_group'=>$this->data['user_info']['m_group'],
                                ':enable' => 1,
                                ':paymentType' => $paymentType
                            ));

                            $paymentLibs = $row['libs'];

                            $this->load->library('payment/'.$paymentLibs, '', 'gateway');
                            $this->gateway->setUid($row['uid']);
                            switch ($paymentFunc){
                                case 'credit':
                                    $log = $this->gateway->creditPay($order_no, $amount);
                                    break;
                                case 'atm':
                                    $log = $this->gateway->atmPay($order_no, $amount);
                                    break;
                                default:
                                    $log = $this->gateway->cvsPay($order_no, $amount, $payment);
                            }
							if($log==NULL){
	...
	public function deposit_result(){
		...
		if($logMsg!=NULL){
			...
			if($this->input->get('order_no')){
				...
				if($row!=NULL){
                    //取出繳費資訊
                    $sqlStr2="select * from `payment_order` where `order_no`='".$row["order_no"]."'";
		...
		if(!$this->agent->is_referral()){
			if($this->input->is_ajax_request()){
				if($this->session->userdata('sms_token') && $this->input->post('sms_token',true)==$this->session->userdata('sms_token')){
					...
					echo json_encode(array('RntCode'=>'N','Msg'=>'Access Token Error'. $this->session->userdata('sms_token')));
				}
			}else{
				echo json_encode(array('RntCode'=>'N','Msg'=>'不允許的方法'));
			}
